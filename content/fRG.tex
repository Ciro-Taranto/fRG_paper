...
\end{align} 
which are the scale derivatives, at fixed self energy, of the product of two Green's functions. %$Q$ is the frequency and momentum transfer.
...
We used the \textit{interaction cutoff}, introduced in Ref. \onlinecite{Honerkamp2004}:
 \begin{equation}
...
%Furthermore it has been shown in Ref.~\onlinecite{Wentzell2016} that, in the context of the single-impurity Anderson model, the vertex-structures do not depend qualitatively on the cutoff-choice, and the specific effect of the interaction-cutoff on the vertex frequenc\mathcal{P}^\Lambda(p,p') &= G^\Lambda(p)S^\Lambda(p') + S^\Lambda(p) G^\Lambda(p'),y structure has been studied in detail.  
