...
This way, it is easy to attribute $\fl{\mathcal{T}}{pp}$ to the flow equation of the only function in Eq. (\ref{eq:decot}) that depends explicitly on $k_1+k_2$, $\dot\phi_{\mathrm{p}}^\Lambda$. 
The same is true for the particle hole crossed channel $\fl{\mathcal{T}}{phc}$ with $\dot\phi_{\mathrm{m}}^\Lambda$, while the particle-hole $\fl{\mathcal{T}}{ph}$ is associated to the third and fourth term on the left hand side of Eq.~(\ref{eq:decot}), $\frac{1}{2}  \dot\phi^{\Lambda}_{\mathrm{m}}(k_2- k_3;k_1,k_2) - \frac{1}{2} \dot\phi^{\Lambda}_{\mathrm{c}}(k_2-k_3;k_1,k_2) $.  
...
For most our calculations we have used the \textit{Interaction cutoff}, introduced in Ref. \onlinecite{Honerkamp2004}: 
 ...
 with $\varepsilon_{\bs{k}} = -2 t [\cos(k_x)+\cos(k_y)] - 4 t' \cos(k_x)\cos(k_y)$. $\mu$ is the chemical potential needed to fix the occupation at the desired value $n$. $\nu$ is a fermionic Matsubara-frequency: $\nu = \frac{\pi}{\beta} (2m+1)$, $m\in \mathbb{Z}$. $\beta=1/T$ is the inverse temperature.       
Correspondingly the interacting Green's function reads: 
\begin{equation}
G^\Lambda (k) = \frac{\Lambda}{i\nu - \varepsilon_{\bs{k}} -\mu-\Lambda\Sigma^\Lambda(k)} 
\end{equation} 
